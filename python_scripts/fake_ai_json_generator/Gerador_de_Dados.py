...
PLANTATION_TYPES = ['Suspenso', 'Reboleira', 'Canteiros Elevados']
LIST_CULTIVARS = ['Cabrillo', 'San Andreas', 'Albion', 'Monterey', 'Portola' , 'Aromas' 'Fronteras' , 'Merced' , 'Camino Real' , 'Camarosa']


class FakeDataGenerator(Faker):
    def __init__(self) -> None:
        Faker().__init__(self, "pt_BR")

    def generate_farmer_data(self) -> list:
        cpf   = self.cpf()
        name  = self.name()
        email = self.email()
        phone = self.phone_number()
        # address = self.address()  | TODO: create an address generator method
        
        return [cpf, name, email, phone]

    def generate_farmers_brazil(self, farmers_quantity: int) -> list[list]: 
        farmers = []

        for i in range(farmers_quantity):
            farmers.append(self.generate_farmer_data())


        return farmers

    def generate_plantatio_data(self) -> list:
        name = self.street_prefix()
        area = random.randint(10, 10000)  # In meters²
        strawberry_plants_quantity = math.floor(area * (7 + random.random()))  # The average number of strawberry plants per area is 7.5
        cultivar = random.choice(LIST_CULTIVARS)
        latitude = self.latitude()
        longitude = self.longitude()
    
        return [name, strawberry_plants_quantity, cultivar, area, latitude, longitude]

    def generate_plantations(self, plantation_quantity) -> list:
        plantation = []
        for i in range(plantation_quantity):
            plantation.append(self.generate_plantatio_data())
...
def generate_mite_quantity(start_quantity: int, growing_rate: float):
    return start_quantity * growing_rate
...
def jsons_insert_query(jsonsList, dataExp):
    ...
    for json in jsonsList:
        ...
        for i in range(1, len(data)):
            print(query, (f"{i}", f"{amostra_id_anterior}", data[i-1]))
            aux.execute(query, (f"{i}", f"{amostra_id_anterior}", data[i-1]))
